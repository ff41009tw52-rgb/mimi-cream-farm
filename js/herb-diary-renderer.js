(() => {
  ...
  function mount(root = document, { onChange = null } = {}) {
    const game = root.getElementById('diaryGame');
    const eventGrid = root.getElementById('eventGrid');
    const choicesEl = root.getElementById('choices');
    const reflectionBox = root.getElementById('reflectionBox');
    const selectedPromptText = root.getElementById('selectedPromptText');
    const customReflectionInput = root.getElementById('customReflectionInput');
    const pageWhisper = root.getElementById('pageWhisper');
    const spread = root.getElementById('spread');
    const controls = root.getElementById('controls');
    const closedUI = root.getElementById('closedUI');
    const prevBtn = root.getElementById('prevBtn');
    const nextBtn = root.getElementById('nextBtn');
    const reopenBtn = root.getElementById('reopenBtn');
    const continueBtn = root.getElementById('continueBtn');
    const dayLabel = root.getElementById('dayLabel');
    const eyebrow = root.getElementById('diaryEyebrow');
    const reflectionHeading = root.getElementById('reflectionHeading');
    const dailyMemory = root.getElementById('dailyMemory');
    const choiceTitle = root.getElementById('choiceTitle');
    ...
    function dayState() {
      ...
      // Preserve the state object captured by editable event handlers.
      day.state = Object.assign(incoming, {
    ...
    function renderEvents() {
      ...
      (day?.events || []).slice(0, 2).forEach((event, index) => {
        ...
        note.addEventListener('keydown', event => {
          if (event.key === 'Enter' && !event.isComposing) insertLineBreak(note, event);
        });
    ...
    function setOpen(open, { emitState = true } = {}) {
      ...
      if (open) {
        ...
        if (getComputedStyle(game).visibility === 'visible') target?.focus({ preventScroll: true });
      } else {
    ...
    customReflectionInput.addEventListener('keydown', event => {
      if (event.key === 'Enter' && !event.isComposing) insertLineBreak(customReflectionInput, event);
    });
    ...
    root.querySelectorAll('img').forEach(img => img.addEventListener('error', () => {
      const warning = root.getElementById('assetWarning');
  ...
  window.HerbDiaryRenderer = { mount };
  if (document.getElementById('diaryGame')) window.herbDiary = mount();
})();
