...
html = html.replace('<button id="main-btn">開始實驗：加水</button>\n            <button id="reset-btn" style="background: #6c757d; display: none;">重新開始</button>', '<button id="main-btn">開始實驗：加水</button>\n            <button id="replay-step-btn" type="button">重看這一步</button>\n            <button id="reset-btn" style="background: #6c757d; display: none;">重新開始</button>')
...
html = html.replace('</p>\n                </div>\n            </article>', '</p>\n                        <div class="cat-quote">點我：我先觀察，再把發現告訴大家！</div>\n                </div>\n            </article>', 1)
html = html.replace('</p>\n                </div>\n            </article>', '</p>\n                        <div class="cat-quote">點我：我有一個問題！讓我們動手試試看！</div>\n                </div>\n            </article>', 1)
script06 = '''
    <script>document.querySelectorAll('.char-card').forEach((card) => card.addEventListener('click', () => card.classList.toggle('active')));</script>
'''
...
html = replace_once(html, '<body>', '<body>\n    <aside class="dashboard-07" aria-label="空氣壓縮觀察數值"><div class="metric-card">現在空氣體積：<span id="volumeValue">0 mL</span></div><div class="metric-card">空氣擠得多不多：<span id="crowdingValue">鬆鬆的</span></div><div class="prediction-card">預測：把活塞往下壓時，空氣粒子會變得比較分散，還是比較擠？<br><button type="button" onclick="answerPrediction07(\'分散\')">比較分散</button><button type="button" onclick="answerPrediction07(\'擠\')">比較擠</button><div id="predictionFeedback07"></div></div><div class="conclusion-card" id="conclusion07">結論：空間變小時，空氣粒子會靠得更近、跑得更快。</div></aside>', '07.html')
html = html.replace('橡皮擦', '橡皮塞')
html = html.replace("missionDesc.innerHTML = \"空氣彈回 20ml 了！證明<span style='color: #e74c3c; font-weight: bold; font-size: 1.1rem;'>空氣可以被壓縮</span>。\";", "missionDesc.innerHTML = \"空氣彈回 20ml 了！證明<span style='color: #e74c3c; font-weight: bold; font-size: 1.1rem;'>空氣可以被壓縮</span>。<br>結論：空間變小時，空氣粒子會靠得更近、跑得更快。\"; document.getElementById('conclusion07')?.classList.add('show');")
html = html.replace('resetBtn.style.display = "none";', 'resetBtn.style.display = "none"; document.getElementById(\'conclusion07\')?.classList.remove(\'show\');')
extra07 = '''
        function updateDashboard07() {
            ...
            if (crowdingEl) { crowdingEl.textContent = isBlocked && currentVol < 12 ? '擠在一起' : (isBlocked && currentVol < 18 ? '有點擠' : '鬆鬆的'); crowdingEl.style.color = isBlocked && currentVol < 12 ? '#e74c3c' : '#1565c0'; }
            if (targetLine) { targetLine.classList.remove('near', 'success'); const distance = Math.abs(currentVol - 20); if (distance <= 0.8) targetLine.classList.add('success'); else if (distance <= 3) targetLine.classList.add('near'); }
        }
        function answerPrediction07(answer) { const feedback = document.getElementById('predictionFeedback07'); if (!feedback) return; if (answer === '擠') { feedback.textContent = '✅ 沒錯！空間變小時，空氣粒子會比較擠。'; feedback.style.color = '#2e7d32'; } else { feedback.textContent = '再想想：活塞往下壓，空氣的空間會變小喔。'; feedback.style.color = '#c2410c'; } }
'''
html = inject_before(html, '        function varColor(name) {', extra07, '07.html')
html = html.replace('currentVol = vol;\n\n            let topPx', 'currentVol = vol;\n            updateDashboard07();\n\n            let topPx')
write('07.html', html)
...
html = replace_once(html, '    <div id="root"></div>', '    <div id="root"></div>\n    <section class="guided-card-08" aria-label="電路跟著做任務"><h2>💡 跟著做模式</h2><ol><li>先把電池的＋端連到燈泡下面的小圓點。</li><li>再把燈泡旁邊的金屬螺紋連回電池－端。</li><li>打開電源，觀察燈泡是否亮起。</li></ol><div class="explain">成功亮燈的原因：電流走成一個完整的圈圈。</div></section>', '08.html')
...
if old_run:
    new_run = '''                // --- 核心邏輯 ---
            const runSimulation = () => {
                ...
                const adj = {};
                components.forEach(c => { if (c.type === COMPONENT_TYPES.BATTERY) { adj[`${c.id}:pos`] = []; adj[`${c.id}:neg`] = []; } else { adj[`${c.id}:thread`] = []; adj[`${c.id}:tip`] = []; } });
                ...
                batteries.forEach(battery => { const fromPos = reachableFrom(`${battery.id}:pos`); const fromNeg = reachableFrom(`${battery.id}:neg`); if (fromPos.has(`${battery.id}:neg`) || fromNeg.has(`${battery.id}:pos`)) shortCircuit = true; components.filter(c => c.type === COMPONENT_TYPES.BULB).forEach(bulb => { const thread = `${bulb.id}:thread`; const tip = `${bulb.id}:tip`; if ((fromPos.has(thread) && fromNeg.has(tip)) || (fromPos.has(tip) && fromNeg.has(thread))) lit.add(bulb.id); }); });
    ...
    print('WARN cannot locate runSimulation in 08.html')
write('08.html', html)
...
html = html.replace('this.state.score >= this.state.targetScore', 'this.state.completedBadges.size >= this.state.targetScore')
html = html.replace('if (this.state.missionItemsCollected >= this.state.currentMissionTarget) {', 'this.state.completedBadges.add(this.state.currentMission.type);\n            if (this.state.missionItemsCollected >= this.state.currentMissionTarget) {')
html = html.replace('scoreEl.textContent = this.state.score;', "scoreEl.textContent = `${this.state.completedBadges.size} / 3`;")
html = html.replace('橘咪咪與白奶油的實驗室', '酸鹼分類接接樂')
...
# Add visible badge explanation on start screen.
html = html.replace('</div>\n        <p style="color: #f1c40f;', '</div>\n        <div style="font-size:20px; color:#fff; line-height:1.8; background:rgba(0,0,0,.25); padding:12px 18px; border-radius:14px;">🏅 酸性小達人　🏅 鹼性小達人　🏅 中性小達人</div>\n        <p style="color: #f1c40f;', 1)
write('09.html', html)
