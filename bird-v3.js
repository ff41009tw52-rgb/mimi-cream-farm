...
function imageFromFile(file) {
  return new Promise((resolve, reject) => {
    ...
    reader.onload = () => {
      const img = new Image();
      img.onerror = () => reject(new Error('瀏覽器無法解析這張圖片'));
...
function canvasData(canvas, quality=.65) {
...
async function compressImage(file) {
  ...
  let full = resizeCanvas(img, 900);
  const thumb = resizeCanvas(img, 280);
  let q = .66;
  let imageData = canvasData(full, q);
  let loops = 0;
  while (imageData.length > 320000 && loops < 12) {
    q = Math.max(.40, q - .05);
    if (q <= .45 && imageData.length > 320000) {
      const smaller = document.createElement('canvas');
      smaller.width = Math.max(1, Math.round(full.width * .86));
      smaller.height = Math.max(1, Math.round(full.height * .86));
      smaller.getContext('2d', { alpha: false }).drawImage(full, 0, 0, smaller.width, smaller.height);
      full = smaller;
    }
    imageData = canvasData(full, q);
    loops++;
  }
  const thumbnailData = canvasData(thumb, .58);
  if (imageData.length > 360000 || thumbnailData.length > 100000 || imageData.length + thumbnailData.length > 440000) {
...
async function runLiveSelfCheck() {
  if (!auth.currentUser || auth.currentUser.uid !== ADMIN_UID) {
    systemCheck = {ok:false, text:'管理員登入狀態不正確'};
    return systemCheck;
  }
  const suffix = `${Date.now()}_${Math.random().toString(36).slice(2,8)}`;
  const birdRef = d('birds', `__diag_${suffix}`);
  const photoRef = d('birdPhotos', `__diag_${suffix}`);
  try {
    await setDoc(birdRef, {_diagnostic:true, name:'系統檢查', shortDescription:'temporary', createdAt:serverTimestamp(), updatedAt:serverTimestamp(), coverThumb:''});
    const a = await getDoc(birdRef);
    if (!a.exists()) throw new Error('鳥類文件寫入後無法讀回');
    const tiny = 'data:image/gif;base64,R0lGODlhAQABAIAAAAAAAP///ywAAAAAAQABAAACAUwAOw==';
    await setDoc(photoRef, {_diagnostic:true, birdId:birdRef.id, imageData:tiny, thumbnailData:tiny, caption:'', order:1, createdAt:serverTimestamp()});
    const b = await getDoc(photoRef);
    if (!b.exists()) throw new Error('圖片文件寫入後無法讀回');
    await deleteDoc(photoRef);
    await deleteDoc(birdRef);
    systemCheck = {ok:true, text:'Firebase 登入、鳥類寫入、圖片寫入與刪除皆正常'};
  } catch (err) {
    try { await deleteDoc(photoRef); } catch {}
    try { await deleteDoc(birdRef); } catch {}
...
function adminPage() {
  ...
  flash = '';
  document.querySelector('#checkBtn').onclick = async () => { systemCheck={ok:true,text:'檢查中…'}; render(); await runLiveSelfCheck(); render(); };
...
async function deleteObservation(id, silent=false) {
  ...
  if (!silent) { await loadData(); flash='觀察紀錄已刪除。'; view='admin'; render(); }
}
...
function birdForm() {
  ...
  document.querySelector('#bf').onsubmit = async e => {
    ...
    let birdRef = editing ? d('birds', editing.id) : doc(col('birds'));
    try {
      if (!auth.currentUser || auth.currentUser.uid !== ADMIN_UID) throw new Error('管理員登入已失效，請重新登入。');
      const data = {
      ...
      for (let i=0;i<items.length;i++) {
        status.textContent=`正在壓縮照片 ${i+1}/${items.length}…`;
        compressed.push({...await compressImage(items[i].file),caption:items[i].caption});
      }
      status.textContent='正在寫入鳥類基本資料…';
      if (editing) await updateDoc(birdRef, data);
      else await setDoc(birdRef, {...data, createdAt:serverTimestamp(), coverThumb:''});
      const verifyBird = await getDoc(birdRef);
      if (!verifyBird.exists()) throw new Error('鳥類基本資料寫入後驗證失敗');

      for (let i=0;i<compressed.length;i++) {
        status.textContent=`正在寫入照片 ${i+1}/${compressed.length}…`;
        const z=compressed[i];
        const pRef=doc(col('birdPhotos'));
        await setDoc(pRef,{birdId:birdRef.id,imageData:z.imageData,thumbnailData:z.thumbnailData,caption:z.caption,order:Date.now()+i,createdAt:serverTimestamp()});
        const verifyPhoto=await getDoc(pRef);
        if (!verifyPhoto.exists()) throw new Error(`第 ${i+1} 張照片寫入後驗證失敗`);
      }
      if (compressed[0]) await updateDoc(birdRef,{coverThumb:compressed[0].thumbnailData,updatedAt:serverTimestamp()});
      status.textContent='正在重新讀取資料…';
      await loadData();
      const saved = birds.find(b=>b.id===birdRef.id);
      if (!saved) throw new Error('資料已送出，但重新讀取後找不到剛新增的鳥種。');
      editing=null; flash=`已成功儲存「${saved.name}」${compressed.length?`，照片 ${compressed.length} 張`:''}。`; view='admin'; busy=false; render();
    } catch (err) {
...
function obsForm() {
  ...
  app.innerHTML = `<div class="panel"><button class="ghost" id="back">← 返回管理中心</button><div class="box"><h2>${editing?'編輯觀察紀錄':'新增觀察紀錄'}</h2><form id="of">${obsFields(editing||{})}${filePicker(12)}<div id="saveStatus" class="muted" style="margin:10px 0"></div><div id="saveError"></div><button id="saveObsBtn" class="btn" style="width:100%">儲存觀察紀錄</button></form></div></div>`;
  document.querySelector('#back').onclick = () => go('admin');
  ...
  document.querySelector('#of').onsubmit = async e => {
    ...
    try {
      ...
      const items=getFiles(); const compressed=[];
      for(let i=0;i<items.length;i++){status.textContent=`正在壓縮照片 ${i+1}/${items.length}…`;compressed.push({...await compressImage(items[i].file),caption:items[i].caption});}
      ...
      const verifyObs=await getDoc(obsRef); if(!verifyObs.exists()) throw new Error('觀察資料寫入後驗證失敗');
      const existing=editing?await docsBy('observationPhotos','observationId',editing.id):[];
      let first=existing[0]?.thumbnailData||editing?.firstThumb||'';
      for(let i=0;i<compressed.length;i++){status.textContent=`正在寫入照片 ${i+1}/${compressed.length}…`;const z=compressed[i],pRef=doc(col('observationPhotos'));await setDoc(pRef,{observationId:obsRef.id,birdId:data.birdId,imageData:z.imageData,thumbnailData:z.thumbnailData,caption:z.caption,order:Date.now()+i,createdAt:serverTimestamp()});if(!first)first=z.thumbnailData;}
      await updateDoc(obsRef,{firstThumb:first,photoCount:existing.length+compressed.length,updatedAt:serverTimestamp()});
      await loadData(); if(!observations.find(o=>o.id===obsRef.id)) throw new Error('資料已送出，但重新讀取後找不到剛新增的觀察紀錄。');
      editing=null;flash=`觀察紀錄已成功儲存${compressed.length?`，照片 ${compressed.length} 張`:''}。`;view='admin';busy=false;render();
    } catch(err){busy=false;btn.disabled=false;btn.textContent='儲存觀察紀錄';status.textContent='';errorBox.innerHTML=`<div class="err">儲存失敗：${esc(errorText(err))}<br><small>版本：${VERSION}</small></div>`;}
  };
}
...
onAuthStateChanged(auth, async user => {
  ...
  if (!booted) {
    booted = true;
    try { await loadData(); } catch (err) { app.innerHTML=`<div class="emptybox"><div class="err">資料讀取失敗：${esc(errorText(err))}</div></div>`; return; }
    render();
  } else if (!busy) {
    render();
  }
...
setTimeout(async()=>{if(!booted){booted=true;try{await loadData();render();}catch(err){app.innerHTML=`<div class="emptybox"><div class="err">資料讀取失敗：${esc(errorText(err))}</div></div>`;}}},1800);
