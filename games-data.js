(() => {
  ...
  const games = [
    ...
    },
    {
      ...
      icon: 'fas fa-water',
      title: '🌊 流水搬運小實驗',
      description: '雨後的農場出現了一道小水流，橘咪咪發現礫石、沙和泥土竟然被搬到不同地方，白奶油則追著泥沙一路跑！快來調整水流並仔細觀察，幫兩隻貓咪找出流水侵蝕、搬運與沉積的祕密。',
      publishedAt: '2026-09-13',
      gameNumber: '41',
  ...
  window.FARM_GAMES = Object.freeze(
    games.map((game) => {
      const gameNumber = String(game.gameNumber || '').padStart(2, '0');

      return Object.freeze({
        ...game,
        id: game.id || `game-${gameNumber}`,
        url: `play.html?game=${encodeURIComponent(gameNumber)}`,
